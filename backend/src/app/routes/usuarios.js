...
module.exports = (app) => {
    ...
    app.post('/usuario', (req, res, next)=>{
        let consulta = "INSERT INTO usuario (nombre, username, pass, fecha_nac, sexo) VALUES('"+ req.body.nombre+"','"+req.body.username+"',MD5('"+req.body.pass+"'),'"+req.body.fecha_nac+"','"+req.body.sexo+"')";
        conn.query(consulta, (err, rows, cols) => {
            if(err){
                res.status(500).json({status: 0, mensaje: "Error"})
            } else {
